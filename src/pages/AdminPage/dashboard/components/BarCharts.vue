...
onMounted(() => {
  ...
  themeColors.value = {
    ...
    chart5: computedStyle.getPropertyValue('--chart-5')?.trim() || 'oklch(76.86% 0.16167 113.609)',
...
const totalElectrical = computed(() => permitsData.value.reduce((sum, d) => sum + d.electrical, 0))
const totalElectronic = computed(() => permitsData.value.reduce((sum, d) => sum + d.electronic, 0))
const totalExcavation = computed(() => permitsData.value.reduce((sum, d) => sum + d.excavation, 0))
const totalDemolition = computed(() => permitsData.value.reduce((sum, d) => sum + d.demolition, 0))
const totalMechanical = computed(() => permitsData.value.reduce((sum, d) => sum + d.mechanical, 0))
